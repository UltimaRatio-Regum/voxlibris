...
- **engines/chatterbox/**: HuggingFace Space (Docker SDK) serving Chatterbox TTS as a REST API implementing the VoxLibris TTS API Contract. Files: `app.py` (FastAPI), `Dockerfile`, `requirements.txt`, `README.md`. Voice cloning only (requires reference audio). Emotion control via exaggeration parameter (0.0-1.0) mapped from VoxLibris emotion labels with intensity scaling. CFG weight per emotion. 300 char limit per request, pyrubberband speed/pitch. Requires GPU (T4+). Deploy to HF Spaces, then register the URL in VoxLibris Settings.
